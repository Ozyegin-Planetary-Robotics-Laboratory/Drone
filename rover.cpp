...
#include <arpa/inet.h>
#include <unistd.h>
#include "tmotorcan-cpp/include/tmotor.hpp"  // Adjusted include path
...
int main() {
    ...
    while (true) {
        ...
        // Calculate positions, velocities, and accelerations for each motor
        float front_left_position = linear_velocity + angular_velocity;
        float front_right_position = linear_velocity - angular_velocity;
        float rear_left_position = linear_velocity + angular_velocity;
        float rear_right_position = linear_velocity - angular_velocity;

        int16_t velocity = 100;  // Example velocity value (adjust as needed)
        int16_t acceleration = 50;  // Example acceleration value (adjust as needed)

        // Send position, velocity, and acceleration to motors and log the actions
        try {
            managers[0]->sendPositionVelocityAcceleration(front_left_position, velocity, acceleration);
            std::cout << "Sent front left motor position: " << front_left_position << ", velocity: " << velocity << ", acceleration: " << acceleration << std::endl;

            managers[1]->sendPositionVelocityAcceleration(front_right_position, velocity, acceleration);
            std::cout << "Sent front right motor position: " << front_right_position << ", velocity: " << velocity << ", acceleration: " << acceleration << std::endl;

            managers[2]->sendPositionVelocityAcceleration(rear_left_position, velocity, acceleration);
            std::cout << "Sent rear left motor position: " << rear_left_position << ", velocity: " << velocity << ", acceleration: " << acceleration << std::endl;

            managers[3]->sendPositionVelocityAcceleration(rear_right_position, velocity, acceleration);
            std::cout << "Sent rear right motor position: " << rear_right_position << ", velocity: " << velocity << ", acceleration: " << acceleration << std::endl;
        } catch (const std::exception &e) {
            std::cerr << "Error sending motor position/velocity/acceleration: " << e.what() << std::endl;
            continue;
        }
    }
